...
def MC(Subjects, GlenC, nper, dtf):
    ...
    if p_value < a:
        dt = pd.DataFrame(data={'k': nper, 'r': count, 'p_values': round(p_value, 3), 'Correction': round(
            corrected, 3), 'H_{o}': 'Reject'}, index=[0])
    else:
        dt = pd.DataFrame(data={'k': nper, 'r': count, 'p_values': round(p_value, 3), 'Correction': round(
            corrected, 3), 'H_{o}': 'Fail to Reject'}, index=[0])
...
fig.axes[0].set_xlabel('Mean Difference')
fig.axes[0].axvline(x=obs, color='black', linestyle="--", linewidth=1)
fig.axes[0].axvline(x=-obs, color='black', linestyle="--", linewidth=1)
fig.axes[0].text(0.021, 30, str(obs), rotation=90,

...
def MC(Subjects, GlenC, nper, dtf):
    ...
    print('Number of observations that are >= than the observed std in ' +
...
res0 = calc_diff(dtf_NC, dtf_DC, 'Belief', 3)
...
fig.axes[0].set_xlabel('Standard Deviation Difference')
fig.axes[0].axvline(x=obs, color='black', linestyle="--", linewidth=1)
fig.axes[0].axvline(x=-obs, color='black', linestyle="--", linewidth=1)
fig.axes[0].text(0.022, 30, str(obs), rotation=90,
                 verticalalignment='center', fontweight='bold')
fig.axes[0].text(-0.023, 30, str(-obs), rotation=90,
                 verticalalignment='center', fontweight='bold')
plt.show()
